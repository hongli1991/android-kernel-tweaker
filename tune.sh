...
apply_cpu_caps() {
  ...
  for p in $policies; do
    ...
    if [ -n "$max_set" ]; then
      ...
      su="$p/schedutil"
      # tuned from common open-source Qualcomm WALT/schedutil profiles, balanced for sustained smoothness
      case "$i" in
        1)
...
apply_gpu_caps() {
  ...
  # fallback for kgsl legacy nodes
...
apply_walt_sched() {
  # WALT / scheduler knobs (Qualcomm oriented)
